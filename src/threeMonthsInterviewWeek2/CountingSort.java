...
//        List<Integer> input = Arrays.asList(1,1,3,2,1);
        System.out.println(countingSort(input).toString());
    }
    ...
    public static List<Integer> countingSort(List<Integer> arr) {
        ...
        for (int i = 0; i < arr.size(); i++) {
            int index = arr.get(i); // get value as index
            int value = frequency.get(index); // get value at that index
            frequency.set(index, ++value); // increment value by 1 and replace at same index
        }
        return frequency;
    }
